...
object Application extends Controller {
  ...
  def showPrincipalPage = Action { request =>
    ...
    request.session.get("usuarioEmail").map { userEmail =>
        ...
        var notificationsList: List[Notificacion] = notificacionDao.getNotificationsByUser(currentUser.getId)
        Ok(html.principal(currentUser,notificationsList))
      }.getOrElse {
        Ok(views.html.index()).withNewSession
      }
  }
  ...
  def showAlbumContent(albumId: Long) = TODO
  ...
  def confirmFriendship(friendId: Long, notificationId: Long) = Action {
    ...
    var newNotification: Notificacion = new Notificacion()
    newNotification.setContenido(currentUser.getPrimerNombre +" " + currentUser.getPrimerApellido + " ahora es amigo de " + friend.getPrimerNombre +" " + friend.getPrimerApellido )
  ...
  def newAlbumNotification()  {
    ...
    newNotification.setContenido(currentUser.getPrimerNombre +" " + currentUser.getPrimerApellido +  " a creado un nuevo Album")
