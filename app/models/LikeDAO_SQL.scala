...
class LikeDAO_SQL extends LikeDAO {
      ...
      DB.withConnection { implicit connection =>
        SQL(
          """
            ...
            'fkUsuario -> newLike.getFkUsuario).executeUpdate()
  ...
  override def deleteAlbumLike(idUsuario: Long, idAlbum: Long) {
    ...
    try {
      DB.withConnection { implicit connection =>
        SQL("delete from tlike where fk_usuario = {idUsuario} and fk_album = {idAlbum} and tipo = 'L' ").on('idUsuario -> idUsuario,
          'idAlbum -> idAlbum).executeUpdate()
  ...
  override def deleteAlbumUnlike(idUsuario: Long, idAlbum: Long) {
    ...
    try {
      ...
      DB.withConnection { implicit connection =>
        SQL("delete from tlike where fk_usuario = {idUsuario} and fk_album = {idAlbum} and tipo = 'U' ").on('idUsuario -> idUsuario,
          'idAlbum -> idAlbum).executeUpdate()
  ...
  override def deleteCommentLike(idUsuario: Long, idComment: Long) {
    ...
    try {
      DB.withConnection { implicit connection =>
        SQL("delete from tlike where fk_usuario = {idUsuario} and fk_comentario = {idComment} and tipo = 'L' ").on('idUsuario -> idUsuario,
          'idComment -> idComment).executeUpdate()
  ...
  override def deleteCommentUnlike(idUsuario: Long, idComment: Long) {
    ...
    try {
      DB.withConnection { implicit connection =>
        SQL("delete from tlike where fk_usuario = {idUsuario} and fk_comentario = {idComment} and tipo = 'U' ").on('idUsuario -> idUsuario,
          'idComment -> idComment).executeUpdate()
